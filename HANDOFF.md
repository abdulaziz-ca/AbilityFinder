...
| `../src/link-check.js` | **Phase 5A monitor.** A bounded rotating batch runs every 3 hours and merges last-known results into KV, so the catalog can grow beyond 50 links without exceeding the Free-plan request limit. Reports `broken` / `unreachable` / `inconclusive` / `redirected` as distinct states — see §9; conflating them produces false alarms and a report nobody reads. |
...
| `../scripts/gen-benefits-context.js` | The generator. Redacts figures from the AI grounding on purpose (§9) and emits the monitor's canonical link list. |
| `../wrangler.jsonc` | Deploy config: worker name, `assets.directory = ./public`, `AI` binding, `ASK_LIMIT` rate limit, `LINK_HEALTH` KV, 3-hour link-monitor cron. **No secrets.** |
